...
import chothuematbang from '../../data/matbang.json'
...
//viết hashPassword để res password mã hóa
    ...
    try {
        ...
        dataBody.forEach(cate => {
            cate.body.forEach(async (item) => {
                let postId = v4()
                let attributesId = v4()
                let labelCode = generateCode(item?.overview?.content.find(i => i.name === "Chuyên mục:")?.content)
                ...
                await db.Overview.create({
                    ...
                    Cate: item?.overview?.content.find(i => i.name === "Chuyên mục:")?.content,
