...
const Address = ({ setPayload, invalidFields, setInvalidFields }) => {
    ...
    // hien thong tin dia chi sua
    useEffect(() => {
        if (dataEdit) {
            let addressArr = dataEdit?.address?.split(',')
            let foundPrivince = provinces?.length > 0 && provinces?.find(item => item.province_name === addressArr[addressArr.length - 1]?.trim())
            setProvince(foundPrivince ? foundPrivince.province_id : '')
        }
    }, [provinces, dataEdit])
